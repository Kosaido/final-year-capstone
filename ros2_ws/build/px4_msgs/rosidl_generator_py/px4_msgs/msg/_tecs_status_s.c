...
bool px4_msgs__msg__tecs_status__convert_from_py(PyObject * _pymsg, void * _ros_message)
{
  ...
  {  // weight_ratio
    PyObject * field = PyObject_GetAttrString(_pymsg, "weight_ratio");
    if (!field) {
      return false;
    }
    assert(PyFloat_Check(field));
    ros_message->weight_ratio = (float)PyFloat_AS_DOUBLE(field);
    Py_DECREF(field);
  }
...
PyObject * px4_msgs__msg__tecs_status__convert_to_py(void * raw_ros_message)
{
  ...
  {  // weight_ratio
    PyObject * field = NULL;
    field = PyFloat_FromDouble(ros_message->weight_ratio);
    {
      int rc = PyObject_SetAttrString(_pymessage, "weight_ratio", field);
      Py_DECREF(field);
      if (rc) {
        return NULL;
      }
